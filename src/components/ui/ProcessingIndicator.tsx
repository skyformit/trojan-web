...
export default function ProcessingIndicator({
  ...
  return (
    <div className="relative flex items-start gap-3 pl-1">
      ...
      <div className="relative z-10 max-w-[91%] rounded-[22px] rounded-bl-[18px] border border-[color:rgba(44,53,97,0.14)] bg-white/97 px-5 py-3.5 text-[14px] leading-6 text-slate-800 shadow-[0_18px_40px_rgba(44,53,97,0.08)] ring-1 ring-[color:rgba(44,53,97,0.06)]">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 space-y-0.5">
            <p className="font-sans font-semibold text-slate-900">{label}</p>
            <p className="font-sans text-[10px] leading-[1.35] text-slate-500">{description}</p>
          </div>
        </div>
        <div className="mt-2.5 flex items-end justify-between gap-3 text-[9px] font-mono tracking-[0.22em] text-slate-400">
          <span className="normal-case">Encrypted • end-to-end</span>
          <span className="tracking-[0.16em] text-slate-500">now</span>
        </div>
      </div>
    </div>
  );
